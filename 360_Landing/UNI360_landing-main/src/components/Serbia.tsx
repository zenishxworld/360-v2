...
import { GraduationCap, MapPin, Clock, Users, Globe, BookOpen, Banknote } from 'lucide-react';

interface SerbiaProps {
  selectedCountries: string[];
}

export const Serbia = ({ selectedCountries }: SerbiaProps) => {
  // Don't render if Serbia is not selected
  if (!selectedCountries.includes('serbia')) {
    return null;
  }
  ...
  const highlights = [
    {
      ...
      description: "From just €1,500/year at public universities"
    },
    {
      icon: <GraduationCap className="w-4 h-4" />,
      title: "Recognized Degrees",
      description: "Bologna Process compliant, EU-recognized"
    },
    {
      icon: <Users className="w-4 h-4" />,
      title: "Growing Indian Community",
      description: "Strong community support in Belgrade"
    },
    {
      icon: <BookOpen className="w-4 h-4" />,
      title: "No IELTS Required",
      description: "English marks from high school accepted"
    }
  ];

  const quickFacts = [
    { label: "Average Salary", value: "€8,000-12,000/yr" },
    { label: "Language", value: "Serbian & English" },
    { label: "Living Cost", value: "€150-200/month" }
  ];
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-0">
          ...
          <img 
            ...
            onError={(e) => {
              ...
              e.currentTarget.parentElement!.innerHTML = `
                <div class="flex items-center justify-center h-full bg-gradient-to-br from-primary/10 to-accent/10">
                  <div class="text-center space-y-2 sm:space-y-3 p-3 sm:p-4 lg:p-8">
                    <div class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 mx-auto bg-accent/20 rounded-full flex items-center justify-center">
                      <svg class="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M12 2L2 7v10c0 5.55 3.84 9.74 9 10.16V17h2v10.16c5.16-.42 9-4.61 9-10.16V7l-10-5z"/>
                      </svg>
                    </div>
                    <h3 class="text-base sm:text-lg lg:text-xl font-semibold text-gunmetal">Study in Serbia</h3>
                    <p class="text-xs sm:text-sm lg:text-base text-muted-foreground">Affordable European Education</p>
                  </div>
                </div>
              `;
            }}
        ...
        <div className="p-3 sm:p-4 lg:p-8 space-y-3 sm:space-y-4 lg:space-y-6 order-2 lg:order-1">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-6 h-4 sm:w-8 sm:h-6 lg:w-10 lg:h-7 rounded shadow-sm flex items-center justify-center overflow-hidden" style={{background: 'linear-gradient(to bottom, #C6363C 0%, #C6363C 33%, #0C4076 33%, #0C4076 66%, #fff 66%, #fff 100%)'}}>
            </div>
            <h2 className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-bold text-gunmetal">
              Study in <span className="text-accent">Serbia</span>
            </h2>
          </div>

          {/* Description - Short on mobile, full on tablet+ */}
          <p className="text-xs sm:hidden text-muted-foreground leading-relaxed">
            Affordable European education with high visa success rates and a growing Indian student community.
          </p>
          <p className="hidden sm:block text-sm lg:text-base text-muted-foreground leading-relaxed">
            Serbia offers one of Europe's most affordable study options with tuition from just €1,500/year,
            a 90%+ visa success rate, and strong Indian community support. With English-taught programs
            and no IELTS requirement for many universities, Serbia is an accessible gateway to European education.
          </p>
          ...
          <div className="space-y-1.5 sm:space-y-3 lg:space-y-4">
            <h3 className="text-sm sm:text-base lg:text-lg font-semibold text-gunmetal flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="hidden sm:inline">Why Choose Serbia?</span>
            </h3>
