...
import { CountryButtons } from "@/components/chat/CountryButtons";
import { CounselingFlow } from "@/components/chat/CounselingFlow";
...
export const FloatingChatbot = () => {
  ...
  const isTyping = state.isAwaitingReply;
  ...
  // Dynamic placeholder based on lead capture step
  const getPlaceholder = () => {
    if (state.leadStep === "ASK_NAME") return "Type your full name...";
    if (state.leadStep === "ASK_WHATSAPP") return "+1 555 123 4567";
    return "Ask Nick about studying abroad...";
  };
  ...
  const handleCounselingAnswer = useCallback(
    (step: string, value: string, label: string) => {
      answerCounseling(step as any, value, label);
    },
    [answerCounseling]
  );
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2 bg-gradient-to-b from-gray-50/50 to-white/50 min-h-0">
              {state.messages.map((msg) => (
                <div key={msg.id}>
                  ...
                  <div className={`flex ${msg.isBot ? "justify-start" : "justify-end"} mb-2`}>
                    <div className={`flex items-start gap-1.5 max-w-[90%] ${msg.isBot ? "" : "flex-row-reverse"}`}>
                      ...
                      <div className={`px-2.5 py-1.5 rounded-2xl shadow-sm transition-all duration-200 hover:shadow-md ${
                        ...
                        <p className={`leading-relaxed break-words whitespace-pre-line ${isMobile ? 'text-xs' : 'text-sm'}`}>
                          {msg.text}
                        </p>
                  ...
                  {/* Country selection buttons after welcome message */}
                  {msg.type === "country-selection" && (
                    <div className="mb-2">
                      <CountryButtons onSelect={handleCountrySelect} />
                    </div>
                  )}

                  {/* Counseling flow buttons */}
                  {msg.type === "counseling-question" && msg.counselingStep && (
                    <div className="mb-2">
                      <CounselingFlow
                        step={msg.counselingStep}
                        onAnswer={handleCounselingAnswer}
                      />
                    </div>
                  )}
                </div>
